...
var upRight,upLeft,downLeft,downRight;  // усі можливі варіанти для дамки
...
function showMoves (piece) {
    ...
    if(!match) {
     return 0 ; // якщо збігів не знайдено; відбувається, коли червоний рухається, і ви натискаєте на чорному
    }


    /*===тепер відповідно до їхнього кольору я встановлюю межі та рух дамки===*/
...
function makeMove (index) {
 ...
 /* =========ПЕРЕВІРКА НАПРАВЛЕНИЯ ХОДУ ІГРОКА (КУДА ВІН МОЖЕ ПОХОДИТЬ)======*/
    ...
    if(mustAttack)  // щоб знати, пропустив чи я ряд чи 2
        multiplier = 2;
    else
        multiplier = 1;
        ...
        if(index == cpy_upRight){
            ...
            if(the_checker[1].color=="white"){
               // перемістити
                ...
                //видалити шашку якщо через неї був виконаний "стрибок"
...
function executeMove (X,Y,nSquare){
    // змістити координату на переміщаєму частину
